...
def dfs(x,y, move_count, apple_count, path):
    if graph[y][x] == 1: 
        apple_count += 1
    ...
    for i in range(4):
        ...
        if 0 <= nx < 5 and 0<= ny < 5 and graph[ny][nx] != -1:
            dfs(nx, ny, move_count + 1, apple_count, path)
    graph[y][x] = before
    path.pop()
...
if len(result) == 0:
    ...
    result.sort()  # 이동 횟수 기준으로 정렬
    min_move, min_path = result[1]
    print(min_move)
