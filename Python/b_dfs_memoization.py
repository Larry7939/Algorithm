...
# DFS와 메모이제이션을 활용하여 각 노드의 서브트리 크기를 한 번만 계산하고 저장
# 시간 복잡도: O(N^2) -> O(N)
def solution(parking):
    ...
    subtree_size = [0] * len(parking)
    count_subtree_node(tree, 0, subtree_size)
    ...
    for key in tree.keys():
        ...
        if tree[key][0] != None and tree[key][1] != None:
            left_subtree_count = subtree_size[tree[key][0]]
            right_subtree_count = subtree_size[tree[key][1]]
            answer += left_subtree_count * right_subtree_count
    return answer

# 재귀호출로 매개변수로 주어진 node를 부모 노드로 하는 서브트리의 노드 수 합산 -> 최악의 경우 모든 노드를 한 번씩 방문하므로 O(N)
def count_subtree_node(tree, node, subtree_size):
    if node is None:
        return 0
    if subtree_size[node] != 0:
        return subtree_size[node]

    left_count = count_subtree_node(tree, tree[node][0], subtree_size)
    right_count = count_subtree_node(tree, tree[node][1], subtree_size)
    subtree_size[node] = left_count + right_count + 1
    return subtree_size[node]
